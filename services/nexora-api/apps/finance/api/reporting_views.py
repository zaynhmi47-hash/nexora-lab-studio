...
from apps.finance.api.serializers import (
    FinanceCategoryBreakdownQuerySerializer,
    FinanceCategoryBreakdownSerializer,
)
from apps.finance.selectors.summary_selectors import get_finance_category_breakdown
from apps.identity.authentication import FirebaseIdentityAuthentication


class FinanceCategoryBreakdownView(APIView):
    ...
    def get(self, request, organization_id):
        query = FinanceCategoryBreakdownQuerySerializer(data=request.query_params)
        query.is_valid(raise_exception=True)
        rows = get_finance_category_breakdown(
            organization_id=organization_id,
            **query.validated_data,
        )
        return success_response(
            FinanceCategoryBreakdownSerializer(rows, many=True).data,
            meta={
