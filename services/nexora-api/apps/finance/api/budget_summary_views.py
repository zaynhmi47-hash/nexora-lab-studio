...
from apps.finance.api.budget_summary_serializers import (
    FinanceBudgetSummaryQuerySerializer,
    FinanceBudgetSummarySerializer,
)
...
class FinanceBudgetSummaryView(APIView):
    ...
    def get(self, request, organization_id):
        query = FinanceBudgetSummaryQuerySerializer(data=request.query_params)
        query.is_valid(raise_exception=True)
        summary = get_finance_budget_summary(
            organization_id=organization_id,
            **query.validated_data,
        )
        return success_response(FinanceBudgetSummarySerializer(summary).data)
