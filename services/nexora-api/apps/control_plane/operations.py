...
from .diagnostics import api_route_diagnostic, database_diagnostic, firebase_configuration_diagnostic, overall_operations_status, route_inventory
...
def _guard(request, permission):
    ...
    try:
        ...
        return None, JsonResponse({"detail": "Control Center security state could not be evaluated.", "correlation_id": _request_correlation_id(request)}, status=500)
    return actor, None
...
class ControlPlaneSecurityOverviewView(View):
    def get(self, request):
        ...
        failed_logins_24h = ControlPlaneAuditEvent.objects.filter(event_type=ControlPlaneAuditEventType.LOGIN_FAILED, occurred_at__gte=failed_since).count()
        bootstrap = ControlPlaneBootstrapState.objects.get(pk=1)
        return JsonResponse({
            "authenticated_as": {"user_id": str(actor.user_id), "email": actor.user.email, "role": actor.role},
            "bootstrap": {"locked": bootstrap.locked, "locked_at": bootstrap.locked_at.isoformat() if bootstrap.locked_at else None},
            "principals": {"total": principals.count(), "owners": role_counts[ControlPlaneRole.OWNER], "platform_admins": role_counts[ControlPlaneRole.PLATFORM_ADMIN], "security_admins": role_counts[ControlPlaneRole.SECURITY_ADMIN], "auditors": role_counts[ControlPlaneRole.AUDITOR], "disabled": disabled_count},
            "failed_logins_24h": failed_logins_24h, "generated_at": now.isoformat(),
        })
...
class ControlPlaneOperationsOverviewView(View):
    def get(self, request):
        ...
        diagnostics = {"database": database_diagnostic(checked_at=checked_at), "firebase": firebase_configuration_diagnostic(checked_at=checked_at)}
        try:
            routes = route_inventory()
            diagnostics["api"] = api_route_diagnostic(routes, checked_at=checked_at)
        except Exception as exc:
            routes = []
            diagnostics["api"] = {"status": "unavailable", "latency_ms": None, "checked_at": checked_at, "details": {"error_type": exc.__class__.__name__}}
        try:
            applications = application_snapshot(routes)
        except Exception as exc:
            applications = []
            diagnostics["api"]["details"] = {**diagnostics["api"]["details"], "application_inventory_error_type": exc.__class__.__name__}
        try:
        ...
        return JsonResponse({
            ...
            "status": overall_operations_status(diagnostics), "checked_at": checked_at,
            "diagnostics": diagnostics, "applications": applications, "recent_requests": requests,
            "actions": {"clear_request_telemetry": actor.has_permission(ControlPlanePermission.OPERATIONS_MANAGE)},
        })
...
class ControlPlaneClearTelemetryView(View):
    ...
    def post(self, request):
        ...
        record_control_plane_audit(event_type=ControlPlaneAuditEventType.TELEMETRY_CLEARED, actor=actor.user, success=True, metadata={"cleared_events": cleared})
