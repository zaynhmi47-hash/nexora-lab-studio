...
export function useGoals(){
 ...
 const refresh=useCallback(async()=>{if(status!=='authenticated'||!api.ready){setGoals([]);return;}setLoading(true);setError(null);try{setGoals(await api.listGoals());}catch(cause){setError(cause instanceof Error?cause:new Error('Failed to load goals.'));}finally{setLoading(false);}},[api,status]);
