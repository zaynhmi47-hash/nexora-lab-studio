...
function mapTransaction(item: ApiTransaction): FinanceTransaction {
  return {
    ...
    reference: item.reference, metadata: item.metadata, createdAt: item.created_at, updatedAt: item.updated_at,
  };
}
...
export function useTransactionApi() {
  ...
  const createTransaction = useCallback(async (input: CreateTransactionInput) => {
    const response = await tenantApi.request<ApiTransaction>('/finance/transactions/', {
      ...
      body: JSON.stringify({ direction: input.direction, amount_minor: input.amountMinor, currency: input.currency ?? 'IDR', category: input.category, description: input.description ?? '', occurred_at: input.occurredAt, reference: input.reference ?? '', idempotency_key: input.idempotencyKey ?? '', metadata: input.metadata ?? {} }),
    });
    return mapTransaction(response.data);
  }, [tenantApi]);
