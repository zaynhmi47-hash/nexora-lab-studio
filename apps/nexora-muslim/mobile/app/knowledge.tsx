...
export default function KnowledgeScreen() {
  ...
  useEffect(() => {
    void mockKnowledgeProvider.getSnapshot().then(setSnapshot);
  }, []);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.notice}>
          ...
          <Text style={styles.noticeText}>
            Production answers must use verified sources. The app should not invent hadith, references, fatwas, or scholarly consensus.
          </Text>
        </View>
        ...
        {snapshot.topics.map((topic) => (
          <Card key={topic.id}>
            ...
            <Text style={styles.sourceCount}>{topic.sourceCount} source{topic.sourceCount === 1 ? '' : 's'}</Text>
          </Card>
        ))}
        ...
        <Text style={styles.footer}>
          Religious guidance can differ by school and qualified scholarship. This prototype is a learning interface, not a religious authority.
        </Text>
      </ScrollView>
    </Screen>
  );
