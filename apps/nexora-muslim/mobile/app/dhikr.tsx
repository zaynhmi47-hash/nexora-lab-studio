...
export default function DhikrScreen() {
  ...
  useEffect(() => {
    ...
    mockDhikrRepository.getAll().then((data) => {
      if (mounted) {
        setItems(data);
        setLoading(false);
      }
  ...
  const totalTarget = useMemo(() => items.reduce((total, item) => total + item.target, 0), [items]);
  const completedGoals = useMemo(
    () => items.filter((item) => item.completed >= item.target && item.target > 0).length,
    [items],
  );
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.summaryRow}>
        <View style={styles.summaryCard}>
          ...
          <Text style={styles.summaryLabel}>counted today</Text>
        </View>
        <View style={styles.summaryCard}>
          <Text style={styles.summaryValue}>{completedGoals}/{items.length}</Text>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>No dhikr goals available.</Text>
        </View>
      ) : items.map((item) => {
          ...
          <View key={item.id} style={styles.card}>
            ...
            <View style={styles.actions}>
              <Pressable
                ...
                style={({ pressed }) => [styles.button, done && styles.buttonDone, pressed && !done && styles.pressed]}
                accessibilityRole="button"
                accessibilityLabel={done ? `${item.title} completed` : `Count one ${item.title}`}
...
const styles = StyleSheet.create({
  ...
  hero: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, backgroundColor: colors.primarySoft, borderRadius: radius.lg, padding: spacing.lg, marginBottom: spacing.md },
  ...
  summaryCard: { flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, padding: spacing.md },
  ...
  button: { flex: 1, minHeight: 46, borderRadius: radius.md, backgroundColor: colors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm },
  ...
  resetButton: { width: 46, minHeight: 46, borderRadius: radius.md, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' },
