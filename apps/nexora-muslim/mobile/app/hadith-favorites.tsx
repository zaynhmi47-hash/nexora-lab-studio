...
export default function HadithFavoritesScreen() {
  ...
  const [items, setItems] = useState<HadithFavorite[]>([]);
  useFocusEffect(useCallback(() => {
    const provider = session?.user.provider === 'firebase' ? nexoraCoreKnowledgeProvider(session) : mockKnowledgeProvider;
    void provider.listHadithFavorites().then(setItems);
  }, [session]));
  return <Screen><ScrollView contentContainerStyle={styles.container}>
    <Pressable onPress={() => router.back()}><Text style={styles.back}>‹ Back</Text></Pressable>
    <Text style={styles.title}>Saved Hadith</Text><Text style={styles.subtitle}>Your personal hadith collection.</Text>
    {items.length === 0 ? <Card><Text style={styles.muted}>No saved hadith yet. Open a hadith and save it here.</Text></Card> : items.map((item) =>
      <Pressable key={item.id} onPress={() => router.push('/knowledge/hadith/' + item.id)}>
        <Card style={styles.card}><Text style={styles.cardTitle}>{item.title}</Text><Text style={styles.meta}>{item.collection} · {item.reference}</Text><Text style={styles.muted}>{item.summary}</Text></Card>
      </Pressable>
    )}
  </ScrollView></Screen>;
}
