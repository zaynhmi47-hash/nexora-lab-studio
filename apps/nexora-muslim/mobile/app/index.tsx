...
const missions = [
  { id: 'quran', label: 'Read 10 Quran ayat', progress: '0 / 10' },
  { id: 'tajwid', label: 'Complete 1 Tajwid lesson', progress: '0 / 1' },
  { id: 'dhikr', label: 'Complete 100x Dhikr', progress: '0 / 100' },
];
...
export default function HomeScreen() {
  ...
  return (
    <Screen>
      ...
      <Pressable
        accessibilityLabel={`Open ${nextPrayer.name} prayer`}
        accessibilityRole="button"
        onPress={() => router.push('/prayer')}
        style={({ pressed }) => [styles.prayerCard, pressed && styles.pressed]}
      >
        <View style={styles.prayerTopRow}>
          <View>
            <Text style={styles.cardLabel}>NEXT PRAYER</Text>
            <Text style={styles.prayer}>{nextPrayer.name}</Text>
          </View>
      ...
      <Pressable
        accessibilityLabel="Continue Quran reading"
        accessibilityRole="button"
      ...
      >
        <Card style={styles.continueCard}>
          ...
          <View style={styles.continueCopy}>
            ...
            <Text style={styles.progressLabel}>78% completed</Text>
          </View>
      ...
      <Card style={styles.missionCard}>
        {missions.map((mission, index) => {
          ...
          return (
            <Pressable
              key={mission.id}
              accessibilityLabel={`${completed ? 'Undo' : 'Complete'} ${mission.label}`}
              accessibilityRole="checkbox"
              accessibilityState={{ checked: completed }}
              onPress={() => toggleMission(mission.id)}
              style={[styles.mission, index < missions.length - 1 && styles.missionBorder]}
            >
              <View style={[styles.check, completed && styles.checkCompleted]}>
                <Ionicons name="checkmark" size={14} color={completed ? colors.white : 'transparent'} />
              </View>
              <View style={styles.missionCopy}>
                <Text style={[styles.missionText, completed && styles.missionCompleted]}>{mission.label}</Text>
                <Text style={styles.missionProgress}>{completed ? 'Completed' : mission.progress}</Text>
              </View>
