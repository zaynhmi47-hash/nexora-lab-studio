      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Card style={styles.referenceCard}>
          ...
          <View style={styles.gradePill}>
            <Text style={styles.grade}>Grade: {gradeLabel[item.grade]}</Text>
          </View>
        </Card>

        <Text style={styles.section}>Summary</Text>
        <Card><Text style={styles.body}>{item.summary}</Text></Card>

        <Text style={styles.section}>Source</Text>
        <Card>
          <Text style={styles.sourceTitle}>{item.source.title}</Text>
          <Text style={styles.meta}>Type: {item.source.type}</Text>
          {item.source.reference ? <Text style={styles.muted}>{item.source.reference}</Text> : null}
          {item.source.url ? <Text style={styles.muted}>External source available</Text> : null}
        </Card>

        <View style={styles.notice}>
          <Text style={styles.noticeTitle}>Verification status</Text>
          <Text style={styles.muted}>This prototype intentionally does not display fabricated Arabic text or translations. A production adapter must preserve the original text, collection, reference, grading, and source provenance.</Text>
        </View>
      </ScrollView>
    </Screen>
  );
...
const styles = StyleSheet.create({
  ...
  referenceCard: { marginTop: spacing.xl },
  collection: { color: colors.primaryDark, fontWeight: '900', fontSize: 17 },
  reference: { marginTop: spacing.xs, color: colors.textMuted, lineHeight: 21 },
  ...
  grade: { color: colors.primaryDark, fontWeight: '800', fontSize: typography.caption },
