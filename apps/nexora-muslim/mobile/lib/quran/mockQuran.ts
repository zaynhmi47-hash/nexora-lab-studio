...
const pages: Record<number, QuranPage> = { 2: { surah: surahs[1], ayahs: [{ surahNumber: 2, numberInSurah: 153, arabicText: 'يَا أَيُّهَا الَّذِينَ آمَنُوا اسْتَعِينُوا بِالصَّبْرِ وَالصَّلَاةِ', translation: 'O believers! Seek comfort in patience and prayer.' }] } };
...
function statistics(): QuranReadingStatistics { const todayKey = new Date().toISOString().slice(0,10); const todayLogs=logs.filter((item)=>item.date===todayKey); const pagesToday=todayLogs.reduce((sum,item)=>sum+item.pages,0); const minutesToday=todayLogs.reduce((sum,item)=>sum+item.minutes,0); return { goal, today:{pages:pagesToday,minutes:minutesToday,pagesProgress:goal.dailyTargetPages?Math.min(pagesToday/goal.dailyTargetPages,1):0,minutesProgress:goal.dailyTargetMinutes?Math.min(minutesToday/goal.dailyTargetMinutes,1):0},currentStreak:logs.length?1:0,total:{pages:logs.reduce((sum,item)=>sum+item.pages,0),minutes:logs.reduce((sum,item)=>sum+item.minutes,0)}}; }
export const mockQuran: QuranPort = {
  async listSurahs(){return surahs;}, async getSurah(surahNumber){return pages[surahNumber]??null;}, async getReadingPosition(){return readingPosition;}, async saveReadingPosition(position){readingPosition=position;},
  async listBookmarks(){return bookmarks;}, async saveBookmark(bookmark){bookmarks=[bookmark,...bookmarks.filter((item)=>item.id!==bookmark.id)];return bookmark;}, async removeBookmark(bookmarkId){bookmarks=bookmarks.filter((item)=>item.id!==bookmarkId);}, async listRecitations(){return recitations;},
  async getReadingGoal(){return goal;}, async updateReadingGoal(value){goal=value;return goal;}, async getReadingStatistics(){return statistics();}, async logReading(log){logs.unshift(log);return log;},
};
